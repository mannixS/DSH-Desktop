...
| `app-updater.js` | **客户端自身程序自动更新**。基于 electron-updater，**多更新源**（GitHub Releases / CNB 国内镜像 / 自定义 URL，`auto` 模式并发测速择优）；Windows 走官方 `quitAndInstall`，**macOS 为引导式手动更新**（无 Apple 证书时 ShipIt 签名校验必然失败，故把安装包复制到「下载」并打开，由用户拖拽覆盖）；仅在打包环境启用 |
