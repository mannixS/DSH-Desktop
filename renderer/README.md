...
| dsh 工作台 | 内嵌 `<webview>` 加载 dsh Web UI；**优先加载 dsh 打印的认证 URL**（新内核含 `?token=`，用于兑换签名 cookie），无则回退 `127.0.0.1:端口` |
| 设置 | 自动检查开关、自动安装、检查间隔、更新通道（latest/stable）、启动模式、端口；程序更新（**更新源**：自动测速 / GitHub / CNB 国内镜像 / 自定义，检查、下载/安装，macOS 为引导式手动更新） |
